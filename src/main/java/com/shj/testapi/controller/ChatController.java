...
public class ChatController {
    ...
    public void sendMessage(@Payload ChatRequestDto chatRequestDto) {
        ...
        rabbitTemplate.convertAndSend(CHAT_EXCHANGE_NAME, "room." + chatResponseDto.getRoomId(), chatResponseDto);  // '/exchange/chat.exchange/room.{roomId}' 이 클라이언트 주소로 상시 켜져(구독되어)있는 스톰프(웹소켓) 프론트엔드에 메세지 전달.
    }
...
//    @RabbitListener(queues = CHAT_QUEUE_NAME)  // 이를 사용하면, 메시지가 프로듀서(백엔드)에서 컨슈머(프론트엔드)로 이동하는 과정에서, 큐에 도착할 때 메소드가 자동 호출되도록 할 수 있다. (즉, 백엔드에서 메세지 발행된 후 호출됨.)
